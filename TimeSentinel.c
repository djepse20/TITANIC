...
CsvField* CsvFieldFromFile(FILE* csvFile)
{
   ...
   // allokere et array af integer der svarer til hvor mange characters hver linje har
    int* numberOfChars = GetAmountOfCharacters(csvFile, employeeCount);
...
char** ParseCsv(int numberOfChars, FILE* csvFile)
{
    ...
    // NB! der ligges en til for at tage højde for den nulterminerede char som bliver tilføjet af fgets ved indlæsning af tekst linen
    ...
    // da csv lines ikke bruges andre steder end her free's dens plads også her. 
    free(csvLine);
    
    return parsedLine;
}

// NB! Nedenstående kode er afhængig af at csvFile afsluttes med en newline
...
int GetNumberOfLines(FILE* csvFile)
{
    ...
    // bruges til at sætte sætte file pointeren til starten af filen igen   
    rewind(csvFile);

    return numberOfCsvLines;
}
// retunere et array af antallet af carachter for hver linje
int* GetAmountOfCharacters(FILE* csvFile, int numberOfLines)
{
    int n = 0;
...
Employee* AllocateStructs(CsvField* csvField)
{
    ...
    for (int i = 0; i < employeeCount; i++)
    {
        ...
        // ligger antallet af vagter over i csvField Struct felte shiftCount for at kunne iterere over vagterne i senere funktioner
...
int CountTotalShifts(char** CsvString)
{
    ...
    while (*CsvString)
    {
        //hvis situations tegn findes er der tale om et medarbjeder navn og ikke en vagt, der sammenlignes også med NULL da at denne indikere at der here ikke er tale om en vagt
...
{
    for (int i = 0; i < employeeCount; i++)
    // convetere her de forskellige datoer til sekunder i unix tid og læser dem ind i vagt structs 
...
void ParseShiftForEmployee(CsvField* csvField, Employee* employee, int employeeIndex)
{
    ...
    for (int i = 0; i < csvField[employeeIndex].shiftCount; i++)
    {
        ...
        //NB! addere her med INDEX_SHIFT_START som svarer til 1 for at skippe det første csv felt som svarer til medarbejder navnet
        if (strcmp(csvField[employeeIndex].fieldsPerLine[i + INDEX_SHIFT_START],"NULL"))
        {
            ...
            // NULL bruges her til at indikere der stadig læses fra det samme carachter array
            endTime = strtok(NULL, "-");
            //  scanner her tiden ind i arrayet af timestamps som svarer til det givne tidspukt
            ScanTimestamp(startTime, timestamp);
            // her kalkuleres starttiden i unix tid ud fra timestamp integer arrayet
...
// her kalkuleres tiden i unix tid for et givent statisk allokere integer array der repræsentere et tidspunkt
unsigned long long CalculateTimeWorked(int* timestamp)
{
    ...
    UnixTime.tm_isdst = 0;
    //bruger mk time til at returnere unix tiden ud fra struct tm
...
void CalculateTimeBetweenShifts(Employee* employee, CsvField* csvField)
{
    ...
    for (int i = 0; i < employeeCount; i++)
    {
        ...
        {
            //tiden mellem sluttidspunktet på en vagt og starttidspunkt på den forrige vagt i unix tid udregnes her, hvor man så får antallet af sekunder mellem de 2 vagter
            if (j == 0)
            //NB! ved den første vagt sættes tiden før sidste vagt til ULLONG_MAX da at der ikke er nogen vagt før den første vagt
...
void Module_CheckSunday(CsvField* csvField, Employee* employee)
{
    ...
    for (int i = 0; i < employeeCount; i++)
    {
        ...
        for (int j = 0; j < csvField[i].shiftCount; j++)
        {
            ...
            time = localtime((time_t*)&employee[i].shift[j].unixStartTime); 
            ...
            // Her gøres det samme som ovenfor bare foor sluttidspunktet. Dette sikre at man ikke arbejder ind i en søndag heller.
            time = localtime((time_t*)&employee[i].shift[j].unixEndTime);
...
void CheckIfNewWeekAndReset(Employee* employee, struct tm* timeCurrent, struct tm* timeLastShift, Time* time, int i, int j)
{
    if ((timeCurrent->tm_wday == MONDAY) ||
            ...
            timeCurrent->tm_yday - timeLastShift->tm_yday >= DAYS_IN_WEEK))
    {
        /* Hvis udtrykket er sandt så vil hoursWorkWedeekTotal assignes til værdien af den akkumulerede tid,
           ...
           sidst vil endTimeWeek assignes til Unixtime værdien af den forhenværende vagt, 
           da dette var den sidste vagt før det var en ny uge */
        time->hoursWorkedWeekTotal = time->accHoursWorkedWeek;
        time->accHoursWorkedWeek = 0;
        time->endTimeWeek = employee[i].shift[j - 1].unixStartTime;
    }
}
