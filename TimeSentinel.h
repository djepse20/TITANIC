...
#define SECONDS_IN_WEEK       604800     // Antal sekunder pr. uge XDXD
...
typedef struct Time
{
    ...
    unsigned long long endTimeWeek;
...
/* CSV Parsing */
const char* GetInputFilename(); // input navn på csv filen
FILE* OpenCsvFile(); // åbner csv filen og returner pointere til FILE structen
void PrintAssert(int expression, char* str); // bruges til at stoppe programmet i at kører samt at printe en error message ud på skærmen
...
int* GetAmountOfCharacters(FILE* csvFile ,int numberOfLines); /*kalkuler antallet af carachters */
